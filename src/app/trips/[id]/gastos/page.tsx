...
import {
  ArrowLeft, Plus, Trash2, Pencil, Receipt, X, Check,
} from "lucide-react";
...
export default function GastosPage({ params }: Props) {
  const { id } = use(params);
  const { getTrip, addExpense, updateExpense, deleteExpense } = useTripStore();
  ...
  const handleDelete = (expenseId: string, desc: string) => {
    if (confirm(`¿Eliminar "${desc}"?`)) deleteExpense(id, expenseId);
  };
        ...
        {trip.expenses.length === 0 ? (
          ...
          >
            ...
            <p style={{ color: "var(--text-secondary)", marginBottom: "1.25rem" }}>
              Registra el primer gasto del viaje
            </p>
            <button
              onClick={() => setShowForm(true)}
              className="btn-primary"
              style={{ padding: "0.75rem 1.5rem", border: "none", cursor: "pointer", fontSize: "0.9rem" }}
            >
                          ...
                          <div style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
                            ...
                            <span
                              style={{
                                ...
                                background: `${payer?.color ?? "#888"}22`,
                                color: payer?.color ?? "#888",
                                fontWeight: 600,
                              }}
                            >
                              {payer?.name ?? "?"}
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", flexShrink: 0 }}>
                        ...
                        <div style={{ display: "flex", gap: 4 }}>
                          <button
                            ...
                            style={{ padding: 8, borderRadius: "var(--radius-md)", background: "rgba(139,92,246,0.1)", border: "none", color: "var(--accent-violet)", cursor: "pointer" }}
                          >
                            <Pencil size={14} />
                          </button>
                          <button
                            onClick={() => handleDelete(expense.id, expense.description)}
                            style={{ padding: 8, borderRadius: "var(--radius-md)", background: "rgba(244,63,94,0.1)", border: "none", color: "var(--accent-rose)", cursor: "pointer" }}
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
      ...
      <AnimatePresence>
        {showForm && (
          <ExpenseModal
            ...
            onSave={(data) => {
              if (editingId) {
                updateExpense(id, editingId, data);
              } else {
                addExpense(id, data);
              }
              handleClose();
...
function ExpenseModal({
  ...
  trip: NonNullable<ReturnType<ReturnType<typeof useTripStore>["getTrip"]>>;
  ...
  const {
    ...
    defaultValues: editing
      ? {
          ...
          amount: fromCents(editing.amountCents),
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem" }}>
          ...
          <button
            onClick={onClose}
            style={{ padding: 8, borderRadius: "var(--radius-md)", background: "var(--bg-elevated)", border: "none", cursor: "pointer", color: "var(--text-muted)" }}
          >
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit(onSave)}>
          ...
          <div style={{ marginBottom: "1.25rem" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
              ...
              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                {watchedSplit.length} de {trip.participants.length}
              </span>
            </div>
            <Controller
              name="splitAmong"
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          field.onChange(
                            isChecked ? current.filter((id) => id !== p.id) : [...current, p.id]
                          );
                        }}
                        style={{
                          display: "flex",
                          ...
                          padding: "6px 12px",
                          borderRadius: "var(--radius-full)",
                          border: `1px solid ${isChecked ? p.color : "var(--border-default)"}`,
                          background: isChecked ? `${p.color}22` : "var(--bg-elevated)",
                          color: isChecked ? p.color : "var(--text-secondary)",
                          cursor: "pointer",
                          fontSize: "0.8125rem",
          ...
          <div style={{ display: "flex", gap: "0.75rem", marginTop: "0.5rem" }}>
            <button
              type="button"
              onClick={onClose}
              style={{ flex: 1, padding: "0.875rem", borderRadius: "var(--radius-full)", border: "1px solid var(--border-default)", background: "transparent", color: "var(--text-secondary)", cursor: "pointer", fontSize: "0.9rem" }}
            >
              Cancelar
            </button>
            <button
              type="submit"
              className="btn-primary"
              style={{ flex: 2, padding: "0.875rem", border: "none", cursor: "pointer", fontSize: "0.9rem" }}
            >
